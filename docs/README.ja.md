...
このリポジトリは2つのパッケージからなるモノレポです:

- `packages/flowvia-lib` - ネットワーク図を描画する React コンポーネントライブラリ(Webpack でビルド)
...
3. **作業を保存する**:
   - **クイック保存** - お使いのデバイスのローカルストレージに保存
   - **エクスポート** - JSON ファイルとしてダウンロード
   - **インポート** - JSON ファイルから読み込み

### ストレージオプション

- **ローカルストレージ**: 図はブラウザの IndexedDB に保存され、ブラウザを閉じたりデバイスを再起動したりしても保持されます
- **自動保存**: 編集を止めてから5秒後に変更内容を自動保存します
- **エクスポート/インポート**: JSON ファイルとして永続保存
- **サーバーストレージ**(セルフホスト時): `ENABLE_SERVER_STORAGE` を有効にして(デフォルトで有効)Docker で実行している場合、図はホスト側の `./diagrams` にも同期されます
