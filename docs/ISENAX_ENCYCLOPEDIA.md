# Isenax Codebase Encyclopedia

This is an architecture reference for the Isenax codebase: what each package does, how state flows, and where to find things. It describes the current structure only — for what changed and when, see [CHANGELOG.md](../CHANGELOG.md).

---

## Overview

Isenax is a pnpm monorepo containing a React component library for drawing isometric network diagrams (`isenax-lib`), a Progressive Web App that uses this library (`isenax-app`), and an optional Express backend for server-side diagram storage (`isenax-backend`). This encyclopedia provides a comprehensive guide to navigating and understanding the codebase structure, making it easy to locate specific functionality and understand the architecture.

## Table of Contents

1. [Monorepo Structure](#monorepo-structure)
2. [Library Architecture (isenax-lib)](#library-architecture-isenax-lib)
3. [Application Architecture (isenax-app)](#application-architecture-isenax-app)
4. [Backend Architecture (isenax-backend)](#backend-architecture-isenax-backend)
...
Web-Isenax/
├── packages/
│   ├── isenax-lib/             # React component library (published as "isenax" on npm)
...
│   │
│   ├── isenax-app/             # Progressive Web App
│   │   ├── src/                 # App source code
...
│   │   └── tsconfig.json       # TypeScript config
│   │
│   └── isenax-backend/        # Optional backend server
...
## Library Architecture (isenax-lib)

### Entry Points

- **`packages/isenax-lib/src/index.tsx`**: Development mode entry with examples
- **`packages/isenax-lib/src/Isoflow.tsx`**: Main component exported for library usage
...
(See `packages/isenax-lib/src/Isoflow.tsx` for the exact nesting — the outer providers wrap the internal `App` component that renders `Renderer` and `UiOverlay`.)
...
## Backend Architecture (isenax-backend)

### Overview

The backend package is an optional Express.js server that provides filesystem-backed diagram storage, useful for Docker deployments where diagrams should sync across devices/browsers. It is not required to run Isenax — the app works fully offline without it (see [Application Architecture](#application-architecture-isenax-app)).

**Location**: `/packages/isenax-backend/`
...
**App Service** (`packages/isenax-app/src/services/storageService.ts`):
...
**Location**: `/packages/isenax-lib/src/stores/modelStore.tsx`
**Types**: `/packages/isenax-lib/src/types/model.ts`
...
**Location**: `/packages/isenax-lib/src/stores/sceneStore.tsx`
**Types**: `/packages/isenax-lib/src/types/scene.ts`
...
**Location**: `/packages/isenax-lib/src/stores/uiStateStore.tsx`
**Types**: `/packages/isenax-lib/src/types/ui.ts`

## Application Architecture (isenax-app)

### Overview

The Isenax application is a Progressive Web App built with Rsbuild that provides a complete diagram editor interface using the isenax-lib library.

### Key Components

#### App Entry (`packages/isenax-app/src/index.tsx`)
- Initializes the React app
- Registers the service worker for PWA functionality
- Sets up Quill editor styles
- Initializes i18n

#### Main App (`packages/isenax-app/src/App.tsx`)
- Contains the Isoflow component from isenax-lib
...
#### Service Worker
- **Registration**: `packages/isenax-app/src/serviceWorkerRegistration.ts` — standard CRA-style `register()`/`unregister()`. In production it registers `service-worker.js` directly; on localhost it first validates the script is served correctly before registering, to avoid caching issues during development.
- **Worker script**: `packages/isenax-app/public/service-worker.js` — a hand-written cache-first worker, versioned cache name (`isenax-v3`). rsbuild content-hashes JS/CSS filenames (e.g. `index.<hash>.js`), so those can't be precached by a fixed path without a build-time manifest step; `install` only precaches the unhashed `public/` assets (`manifest.json`, favicon/logo files). On `fetch` it serves from cache when present, otherwise fetches from the network and lazily caches the (basic, 200-status) response — this is how hashed bundles end up cached, on first real request rather than up front. On `activate` it deletes any cache not matching the current cache name.

### App Features

- **Auto-Save**: Diagram changes are auto-saved 5 seconds after the last edit (`packages/isenax-app/src/App.tsx`), via `storageManager.getStorage().saveDiagram(...)`
- **Persistent Local Storage**: Diagrams are stored in the browser via IndexedDB (`packages/isenax-app/src/services/storageService.ts`), which survives browser restarts — not sessionStorage, and not lost when the tab closes
...
- **Server Storage**: Optional persistent backend storage that syncs across devices when the backend is reachable (see [Backend Architecture](#backend-architecture-isenax-backend))
...
#### Renderer (`packages/isenax-lib/src/components/Renderer/`)
...
### Scene Layers (`packages/isenax-lib/src/components/SceneLayers/`)
...
#### MainMenu (`packages/isenax-lib/src/components/MainMenu/`)
- **Purpose**: Application menu
- **Features**: Open, Export, Clear; fully i18n-translated

#### ToolMenu (`packages/isenax-lib/src/components/ToolMenu/`)
...
#### ItemControls (`packages/isenax-lib/src/components/ItemControls/`)
...
#### TransformControlsManager (`packages/isenax-lib/src/components/TransformControlsManager/`)
...
**Location**: `/packages/isenax-lib/src/config/`
...
Isenax supports multiple languages using `i18next`/`react-i18next`, with automatic browser-language detection and fallback to English.

### Library i18n (`packages/isenax-lib/src/i18n/`)
...
### App i18n (`packages/isenax-app/src/`)
...
- **isenax-lib**: Built with Rslib/Rspack, output as a CommonJS library (see `rslib.config.ts`'s `format: 'cjs'`)
- **isenax-app**: Built with Rsbuild (Rspack-based)
- **isenax-backend**: Node.js ES modules (no build step)

### Build Configurations

#### Library (Rslib)
- **Config**: `/packages/isenax-lib/rslib.config.ts`
...
#### Application (Rsbuild)
- **Config**: `/packages/isenax-app/rsbuild.config.ts`
...
### Test Files Location
- Library unit/integration tests: `packages/isenax-lib/src/**/__tests__/`
- App tests: `packages/isenax-app/src/**/*.test.tsx`
- Test utilities: `packages/isenax-lib/src/fixtures/`
...
### Key Test Areas
- `/packages/isenax-lib/src/schemas/__tests__/`: Schema validation
- `/packages/isenax-lib/src/stores/reducers/__tests__/`: State logic, including connector reducer tests
- `/packages/isenax-lib/src/hooks/__tests__/useHistory.test.tsx`: Undo/redo transaction hook
- `/packages/isenax-lib/src/utils/__tests__/`: Utility functions
...
- Edit files in `packages/isenax-lib/src/`
- The app resolves `isenax` to the library's dev build, so changes are picked up without a manual republish (run `pnpm run dev:lib` for a watch build)

4. **Making App Changes**:
- Edit files in `packages/isenax-app/src/`
- Hot reload updates the browser automatically

5. **Making Backend Changes**:
- Edit `packages/isenax-backend/server.js`
- Restart the server, or use `pnpm run dev:backend` (nodemon) for auto-reload

### Key Development Files

#### 1. Configuration (`packages/isenax-lib/src/config.ts`)
...
#### 2. Hooks Directory (`packages/isenax-lib/src/hooks/`)
...
#### 3. Interaction System (`packages/isenax-lib/src/interaction/`)
...
#### 4. Utilities (`packages/isenax-lib/src/utils/`)
...
#### 5. Type System (`packages/isenax-lib/src/types/`)
...
#### 6. Schema Validation (`packages/isenax-lib/src/schemas/`)
...
- **Location**: `/packages/isenax-lib/src/hooks/useHistory.ts`
...
**Test coverage**: `packages/isenax-lib/src/hooks/__tests__/useHistory.test.tsx` covers the hook directly, and `/e2e-tests` includes real-browser undo/redo scenarios for rectangles/text, connectors, and multi-node operations.
...
**i18n translations?** → `/src/i18n/` (library), `packages/isenax-app/public/i18n/app/` (app)
**Server storage?** → `/packages/isenax-backend/server.js`, `packages/isenax-app/src/services/storageService.ts`
...
**Backend API?** → `/packages/isenax-backend/server.js`
...
| Backend server | `/packages/isenax-backend/server.js` | |
| App storage service | `/packages/isenax-app/src/services/storageService.ts` | IndexedDB + optional server sync |
| App i18n config | `/packages/isenax-app/src/i18n.ts` | |
| App translations | `/packages/isenax-app/public/i18n/app/{lang}.json` | |
