...
3. **Сохранить вашу работу**:
   - **Быстрое сохранение** - Сохраняет в локальное хранилище вашего устройства
   - **Экспорт** - Скачать как JSON файл
   - **Импорт** - Загрузить из JSON файла

### Варианты хранения

- **Локальное хранилище**: Диаграммы сохраняются в IndexedDB вашего браузера и остаются даже после закрытия браузера или перезагрузки устройства
- **Автосохранение**: Автоматически сохраняет изменения через 5 секунд после того, как вы прекращаете редактирование
- **Экспорт/Импорт**: Постоянное хранилище в виде JSON файлов
- **Хранилище сервера** (при самостоятельном хостинге): При запуске через Docker с включённым `ENABLE_SERVER_STORAGE` (включено по умолчанию) диаграммы также синхронизируются с `./diagrams` на хосте
