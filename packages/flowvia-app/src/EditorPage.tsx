...
function EditorPage() {
  ...
  useEffect(() => {
    if (isReadonlyUrl) {
      console.warn('Flowvia is running in read-only display mode. Editing is disabled.');
      ...
      storageManager.initialize().then(async () => {
        try {
          ...
          if (diagramData) {
            ...
            setFlowviaKey(prevKey => prevKey + 1);
          }
        } catch (error) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('flowvia_diagrams');
    if (saved) {
      try {
  ...
  useEffect(() => {
    const autoSaveInterval = setInterval(() => {
      if (hasUnsavedChanges && currentModel && !isReadonlyUrl) {
        ...
        localStorage.setItem('flowvia_autosave', JSON.stringify(autoSaveData));
  ...
  useEffect(() => {
    if (!isReadonlyUrl) {
      const autoSaveData = localStorage.getItem('flowvia_autosave');
      if (autoSaveData) {
        try {
          ...
          if (window.confirm('An auto-saved diagram was found. Would you like to restore it?')) {
            ...
            setFlowviaKey(prevKey => prevKey + 1);
            localStorage.removeItem('flowvia_autosave'); // Clear auto-save after restoring
          }
        } catch (error) {
  ...
  const saveDiagram = () => {
    ...
    localStorage.setItem('flowvia_diagrams', JSON.stringify(updatedDiagrams));
    setCurrentDiagram(newDiagram);
    setShowSaveDialog(false);
    setHasUnsavedChanges(false);
    // Clear auto-save after successful save
    localStorage.removeItem('flowvia_autosave');
  };
  ...
  const loadDiagram = (diagram: SavedDiagram) => {
    ...
    setHasUnsavedChanges(false);
    setFlowviaKey(prevKey => prevKey + 1); // Force re-render Flowvia
  };
  ...
  const exportDiagram = () => {
    ...
    const exportData = {
      name: diagramName || 'flowvia-diagram',
      version: '1.0',
    ...
    a.download = `${diagramName || 'flowvia-diagram'}-${exportTimestamp}.json`;
