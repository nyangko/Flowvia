...
export const StorageManager: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const exportAllDiagrams = () => {
    const diagrams = localStorage.getItem('flowvia-diagrams');
    if (diagrams) {
      const blob = new Blob([diagrams], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `flowvia-backup-${Date.now()}.json`;
      a.click();
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <ul style={{ fontSize: '14px' }}>
            <li>Flowvia diagrams: {formatBytes(storageInfo.diagrams)}</li>
